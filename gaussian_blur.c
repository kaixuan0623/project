        ...
        if (row == 0) { // transforming the 1st row, reads, stores the first three rows.
            ...
            for (int col_pro = 0; col_pro < width; col_pro++) {  // transforming pixel
                if (col_pro == 0) { // the 1st col.
                    q = apply_gaussian_kernel(pixel_matrix[0], pixel_matrix[1], pixel_matrix[2]);
                    fwrite(&q, sizeof(Pixel), 1, stdout);

                } else if (col_pro == width - 1) { // the last col.
                    q = apply_gaussian_kernel(pixel_matrix[0] + (width - 3), pixel_matrix[1] + (width - 3), pixel_matrix[2] + (width - 3));
                    fwrite(&q, sizeof(Pixel), 1, stdout);

                } else {
                    q = apply_gaussian_kernel(pixel_matrix[0] + (col_pro - 1), pixel_matrix[1] + (col_pro - 1), pixel_matrix[2] + (col_pro - 1));
                    fwrite(&q, sizeof(Pixel), 1, stdout);
            ...
            for (int col_pro = 0; col_pro < width; col_pro++) {  // transforming pixel
                if (col_pro == 0) { // the 1st col.
                    q = apply_gaussian_kernel(pixel_matrix[0], pixel_matrix[1], pixel_matrix[2]);  // Assuming the pixel_matrix already stored the info.
                    fwrite(&q, sizeof(Pixel), 1, stdout);
